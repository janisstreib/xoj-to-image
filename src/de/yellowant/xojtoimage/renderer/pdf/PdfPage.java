...
public class PdfPage extends PdfObject {
	...
	public PdfPage(PdfDocument document, double width, double height) {
		...
		pageDict.dict.put(new PdfName("Parent"), new PdfIndirectReference(
				document.pagesObject));
		// PdfDictionary resDict = new PdfDictionary();
		// PdfDictionary fontDict = new PdfDictionary();
		// fontDict.dict.put(new PdfName("F1"), new
		// PdfIndirectReference(fontDefObject));
		...
		pageDict.dict.put(new PdfName("Resources"), new PdfIndirectReference(
				resources));
		pageDict.dict.put(new PdfName("Contents"), new PdfIndirectReference(
				contents));
	...
	void drawLine(double x1, double y1, double x2, double y2) {
		contentsStream.content.append(x1).append(" ").append(y1).append(" m ")
	...
	void drawStroke(double[] x, double[] y) {
		if (x.length != y.length) {
			throw new IllegalArgumentException(
					"x and y have to be of same length");
		}
		if (x.length < 2) {
			throw new IllegalArgumentException(
					"stroke has to have at least two points");
		}
		contentsStream.content.append(x[0]).append(" ").append(y[0])
				.append(" m");
		for (int i = 1; i < x.length; i++) {
			contentsStream.content.append(" ").append(x[i]).append(" ")
					.append(y[i]).append(" l");
		}
		contentsStream.content.append(" S\n");
	}

	void drawStrokeVaryingWidth(double[] x, double[] y, double[] widths) { // TODO
																			// doesn't
																			// look
																			// beautiful
		if (x.length != y.length) {
	...
	void setAlpha(double alpha) {
		...
		contentsStream.content.append(alphaName.getName()).append(" gs\n");
	}
